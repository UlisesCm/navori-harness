...
export const initCommand = defineCommand({
  ...
  args: {
    ...
    "gitignore-harness": {
      type: "boolean",
      description:
        "Gitignore the harness (.claude/, CLAUDE.md, AGENTS.md, progress/) instead of versioning it. Opt-in; in interactive mode you're asked instead.",
    },
  },
  async run({ args }) {
...
/** Harness paths gitignored by the opt-in. navori.config.json is the source of
 * truth and stays versioned — everything here is regenerated by `navori render`. */
const HARNESS_GITIGNORE_ENTRIES = [".claude/", "CLAUDE.md", "AGENTS.md", "progress/"];

/**
 * Offer to gitignore the harness. The default is to VERSION it (say-no), so this
 * only acts when the user opts in — the interactive confirm defaults to false and
 * auto mode requires an explicit --gitignore-harness. Mirrors offerPreCommitHook.
 * No-op outside a git repo (there's no .gitignore worth writing).
 */
async function offerGitignore(
  ...
  if (!wanted && !opts.autoYes) {
    const answer = await p.confirm({
      message: t(opts.lang).gitignoreHarnessPrompt,
      initialValue: false,
    });
    if (p.isCancel(answer)) return;
    wanted = answer;
  ...
  const result = writeGitignoreHarness(cwd);
  if (result.added > 0) {
    p.log.success(t(opts.lang).gitignoreHarnessWritten(result.path, result.added));
  } else {
    p.log.info(t(opts.lang).gitignoreHarnessAlready(result.path));
  }
}

/**
 * Append the harness entries to .gitignore, creating it if absent. Idempotent:
 * only lines not already present are added, under a labelled block, so re-running
 * never duplicates. navori.config.json is intentionally NOT ignored.
 */
function writeGitignoreHarness(cwd: string): { added: number; path: string } {
  const gitignorePath = join(cwd, ".gitignore");
  const existing = existsSync(gitignorePath) ? readFileSync(gitignorePath, "utf-8") : "";
  const present = new Set(existing.split("\n").map((l) => l.trim()));
  const toAdd = HARNESS_GITIGNORE_ENTRIES.filter((e) => !present.has(e));
  if (toAdd.length === 0) return { added: 0, path: ".gitignore" };

  const header = "# navori harness (regenerado desde navori.config.json con 'navori render')";
