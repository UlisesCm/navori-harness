...
/**
 * What `workspaceHarness: "minimal"` stops writing into a workspace (spec 0018).
 * A previous render under `full` — or any navori before this one — left these on
 * disk, and leaving them is worse than the status quo: they keep occupying the
 * repo AND nobody updates them any more. So the render that stops writing them
 * is the render that removes them.
 */
const WORKSPACE_TRIMMED_PATHS = [
  ".claude/agents",
  ".claude/hooks",
  ".claude/context",
  ".claude/settings.json",
  ".mcp.json",
  // `.claude/scripts/` is deliberately ABSENT, and the reason is worth writing
  // down. A plugin script is the ONE file navori generates without a proof of
  // authorship: hooks, agents and skills carry a `navori:managed` marker, and
  // `scriptAssets` is `{ src, dest, exec }` — no id, no marker. So the
  // authorship test can only answer "foreign", and the render would report the
  // same spared files on every run without ever being able to delete them.
  //
  // Widening the test to accept the scripts' `# Generated by @navori/plugin-…`
  // prose line was considered and rejected: it rests a DESTRUCTIVE decision on
  // an unenforced convention, with two silent failure modes (a plugin that
  // omits the line becomes undeletable; a user file that copies it becomes
  // deletable). Comparing rendered bytes instead was rejected for the reason
  // this very spec gives for reusing `planOrphanRemoval`: a second mechanism
  // for one question is a second mechanism that drifts.
  //
  // The root cause is that `scriptAssets` has no `managedId`, which is also why
  // the engine prune leaves a disabled plugin's scripts behind — a gap that
  // predates this spec. Fixing it there makes the single existing mechanism
  // cover scripts too; until then `doctor` reports the leftovers (R6) and the
  // one-time cleanup is the user's, because navori never deletes what it cannot
  // prove it wrote.
] as const;
 ...
 *
 * Reuses `planOrphanRemoval` rather than growing a second deleter: the question
 * — "navori no longer owns this path; delete what it wrote, keep what it
 * didn't" — is exactly the one the engine prune already answers, with the same
 * authorship test and the same kept-with-a-reason report. Two mechanisms for one
 * question is two mechanisms that drift apart.
 *
...
function reconcileTrimmedWorkspace(wsCwd: string, dryRun: boolean): OrphanRemovalPlan {
  ...
  if (!dryRun && plan.remove.length > 0) {
    ...
    // did not want must be recoverable.
    const handle = createBackup(wsCwd, plan.remove, { exclude: [...EPHEMERAL_HARNESS_PATHS] });
...
export interface WorkspaceRenderResult {
  ...
  /**
   * Files removed because `workspaceHarness: "minimal"` no longer writes them
   * here (spec 0018 R4). Empty under `full`, and empty once reconciled.
   */
  trimmed: string[];
  /**
   * Files inside the trimmed paths that navori did NOT write, so they stayed —
   * with the reason (spec 0018 R5). This is the only signal that something in
   * those directories was the user's, so it is reported, never swallowed.
   */
  trimmedKept: OrphanRemovalPlan["keep"];
}
  ...
  async run({ args }) {
    ...
    for (const ws of result.workspaces) {
      ...
      if (ws.trimmed.length > 0) {
        p.log.message(tr.workspaceTrimmed(ws.workspaceName, ws.trimmed.length));
      }
      if (ws.trimmedKept.length > 0) {
        p.log.warn(tr.workspaceTrimmedKept(ws.workspaceName, ws.trimmedKept.length));
        for (const k of ws.trimmedKept) p.log.message(`  ${dim(`${k.path} — ${k.reason}`)}`);
      }
    }
...
/**
 * Print a compact one-line summary of a SUCCESSFUL render (created/updated/
 * conflict/removed/unchanged counts) plus the engine-tree file list and
 * warnings. Shared by every command that renders inline right after writing
 * `navori.config.json` (`init`, `add`) — extracted from `init.ts`'s former
 * private `renderInline` (#974) once `add` became its second caller.
 *
 * Intentionally only the SUCCESS half: each caller's failure contract differs
 * (`init` logs and moves on to the next step; `add`/`remove` signal a non-zero
 * exit with their own localized copy), so callers run `runRender` and check
 * `result.ok` themselves before calling this.
 */
export function printRenderSummary(result: ReturnType<typeof runRender>): void {
...
/**
 * Run the render flow and print its summary — for callers that don't need a
 * process-exit-code contract on failure and just want to inform the user and
 * move on (`init`; a failed render there logs and lets the wizard continue to
 * the next step, e.g. `.prettierignore`). `add`/`remove` need a stricter
 * contract (non-zero exit on failure) and call `runRender` + `printRenderSummary`
 * directly instead of this wrapper.
 */
export function renderInline(cwd: string): void {
  const result = runRender(cwd, false);
  if (!result.ok) {
    p.log.error(result.reason ?? "Render failed");
    return;
  }
  printRenderSummary(result);
}
...
function buildRenderJson(
  ...
  return {
    ...
    workspaces: result.workspaces.map((w) => ({
      ...
      // Spec 0018 R4/R5. Both halves travel together for the same reason the
      // prune's do (#496/#479): a consumer that sees an empty `trimmed` cannot
      // otherwise tell "nothing to trim" from "everything there is yours".
      trimmed: w.trimmed,
      trimmedKept: w.trimmedKept,
    })),
