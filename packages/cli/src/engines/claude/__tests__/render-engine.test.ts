...
describe("renderClaudeEngine — inspected counter + unchanged surface (P0-fix U1+U2)", () => {
  it("reports inspected count on first render and on second", () => {
    ...
    // arithmetic: 47 inspected − the 4 engram sub-blocks = 43 files actually
    ...
    expect(first.written.length).toBe(43);
...
describe("renderClaudeEngine — dry-run", () => {
  it("reports the plan without writing anything", () => {
    ...
    // PreCompact reminder. 43, not 38: scribe expands the default roster to
    ...
    // adds `scoped-gate` (core), plus the `author-skill` workflow skill. `architect` (spec 0026 T19) defaults OFF
    // (phase F review, 2026-09-17) and CONFIG_FULL carries no explicit
    // `harness.architect: true`, so it stays at seven here.
    expect(r.written).toHaveLength(43);
