...
export function globalRenderConfig(config: GlobalConfig): NavoriConfig {
  return {
    ...
    project: {},
    // No harness overrides needed: every harness key defaults `true`, and
    // `architect` (spec 0032 R33) has no toggle left to default `false` —
    // the global scope ships the FULL toolkit (every `CORE_AGENTS` id,
    // `global-plugin.test.ts`'s own invariant) with no override here.
