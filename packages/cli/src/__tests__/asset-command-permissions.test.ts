...
const EXPECTED_PROMPTS: ReadonlyArray<readonly [string, string]> = [
  ...
  [
    "git checkout --ours",
    "resolves a rebase conflict by overwriting the working tree side; `rebase-rerender` orders it only for content already destined to be regenerated by `render --apply`, but the write itself stays behind the prompt like every other conflict resolution",
  ],
  [
    "git checkout --theirs",
