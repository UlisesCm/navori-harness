...
const EXPECTED_PROMPTS: ReadonlyArray<readonly [string, string]> = [
  ...
  [
    "git checkout",
    "resolves a conflicted managed block by overwriting the working tree side — `--ours` during a rebase, `--theirs` during a merge (`rebase-rerender` covers both under this one entry, since either flag is the same write); ordered only for content already destined to be regenerated by `render --apply`, but the write itself stays behind the prompt like every other conflict resolution",
  ],
  [
    "git worktree list",
