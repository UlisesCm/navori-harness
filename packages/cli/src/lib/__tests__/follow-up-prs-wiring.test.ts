 ...
 *     classification has ONE owner (`follow-up-prs`, renamed from
 *     `babysit-prs` in spec 0026 T14), the pilot only cites it.
 */
...
describe("post-PR loop — content invariants (#337, #338)", () => {
  // Covers: R29
  it("follow-up-prs ships and is registered as a workflow skill", () => {
    expect(WORKFLOW_SKILLS).toContain("follow-up-prs");
    // Registered but absent from disk = a render that writes an empty skill.
    expect(read("skills/follow-up-prs.md")).toContain("name: follow-up-prs");
  });

  it("follow-up-prs keeps the code-vs-infra classification", () => {
  ...
  it("follow-up-prs treats external content as data and never dumps a log", () => {
    const skill = read("skills/follow-up-prs.md");
  ...
  it("follow-up-prs never waits on checks and never auto-implements", () => {
    const skill = read("skills/follow-up-prs.md");
  ...
  it("the pilot reads the checks once, cites the skill, and forbids --watch", () => {
    ...
    expect(pilot).toContain("follow-up-prs");
