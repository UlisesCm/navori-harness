...
describe("computeRenderPlan — blocks.exclude", () => {
  it("EXCLUDABLE_BLOCK_IDS covers the hardcoded core blocks (orquestacion, sdd)", () => {
    expect(EXCLUDABLE_BLOCK_IDS).toContain("orquestacion");
    expect(EXCLUDABLE_BLOCK_IDS).toContain("sdd");
  });
