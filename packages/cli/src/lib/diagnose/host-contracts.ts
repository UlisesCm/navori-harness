...
export const HOST_CONTRACTS: readonly HostContract[] = [
  ...
  {
    id: "claude-hook-parallelism",
    claim:
      "Claude Code launches every matching hook of an event in PARALLEL, so the wait " +
      "an event imposes is its slowest hook, not the sum of them. The exception is " +
      "`SessionEnd`, whose hooks share ONE 1.5-second budget — there they compete for " +
      "a total, and the sum is what describes the wait.",
    source:
    ...
    provedBy:
      ...
      "published 1,564.3s of hook cost where the blocking cost was 928.1s (1.69x, " +
      "+636.2s), which is how a hook that saves ~3ms got argued for on latency grounds.",
    enforcedBy:
      "`report.ts`'s `tollGroups` keys on `toolUseId` + `phase`, so only tool phases " +
      "get the `max` treatment and the lifecycle phases — the only events with no " +
      "`toolUseId` — keep their sum. `report.test.ts` pins both halves.",
  },
  {
    id: "codex-hook-parallelism",
    claim:
      "Codex also launches every matching command hook of an event concurrently, but " +
      "its timeouts are PER HOOK (600s default; `SessionEnd`/`Interrupt` 1s, max 3s) — " +
      "there is no shared budget. And its tool phases do not document `agent_id`: " +
    ...
    source:
      ...
      "events list `agent_id`.",
    provedBy:
      "#924 — the `max` rule had to be established as engine-agnostic before navori " +
      "applied it to a store that mixes hosts, and the sub/main split had to degrade " +
      "to SILENCE rather than to '0 subagent firings' on a host that never documents " +
      "the field the split reads.",
    enforcedBy:
      "`signals.ts`'s `hookMisfires` counts only events that CARRY an `agentId`, so a " +
      "Codex store produces no finding instead of a false one; `signals.test.ts` pins " +
      "the silence.",
  },
  {
