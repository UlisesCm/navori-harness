 ...
 * core managed blocks, the stack presets (their own managed/skills subtrees
 * included), and every plugin bundle. Excludes the R38 registries themselves
 * (`engines/shared/roster.ts`), which are meant to name every retired id
 * forever — sweeping them would just fail on their own append-only history.
 */
...
function sweepRoots(): ReadonlyArray<SweepRoot> {
