...
A ticket's work follows the pipeline documented in the `resolve-ticket` skill (the orchestrator). It's not a spec generator: it's a protocol the `leader` runs by invoking agents and skills in order, with objective gates and artifacts in `.claude/progress/`. Phase-to-navori-infrastructure mapping:
