...
1. **Quality gate**: run `navori receipt check --feature <feature> --include-consumed --json`. `"status":"ok"` and `"fresh":true` → cite the receipt as evidence, no re-run needed. Any other result → run {{qualityGate.full}} again (or document debt in `progress/current.md`).
...
5. **Commit**: atomic, in the configured style (`{{commits}}`), landing inside the work PR before it opens — never a `progress/`-only PR; `history.md` can't cite it. No work PR → commit `progress/` alone.
