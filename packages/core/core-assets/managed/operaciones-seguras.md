...
Read-only por default. Antes de mutar datos, esquema o infraestructura (DB, storage, deploys, recursos cloud), leé y proponé; no mutes sin opt-in explícito del usuario para ESTA tarea.
...
- **Si una mutación destructiva es legítima y necesaria**: explicá qué hace y por qué, y dejá que el usuario la confirme o la corra. Nunca la disfraces con variables, subshells o `--no-verify` para saltarte el gate.
