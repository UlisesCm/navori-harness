...
- **Tier by sub-task, not by round.** Mechanical work → low tier; judgment (design, security regex, removal semantics) → high tier; a single fix round can mix both. See `docs/recipes/model-tiering.md`.
...
Instruct subagents to **write to `.claude/progress/<file>.md`**; you receive only `done -> file`. That folder is ONLY for ephemeral agent handoffs (`audit_*`, `explore_*`, `research_*`, `impl_*`, `review_*`); **session state** (task, plan, blockers) lives in `progress/current.md` (root, git-persisted) and you consolidate it, never the subagents — each `implementer` reports its state (including `blocked`) in its own `impl_<feature>.md`. Verify the diff/evidence yourself; don't trust the report blindly. To close the cycle, invoke `commit-pr-pilot` — when `review_<feature>.md` says `APPROVED` (R2+), or directly for a trivial R1 diff that never went through a `reviewer`. The pilot gates the PR on `{{qualityGate.full}}` (green over the shipping diff — the reviewer's Pass-2 evidence in R2+, or the pilot's own run in R1). Pre-flight: clean working tree, not on `{{branchBase}}`, `gh auth status` ok. If `CHANGES_REQUESTED`, launch a **fresh** `implementer` scoped to just the findings — not a resume of the hot one (dragging a large transcript re-feeds its whole history every turn and rarely pays for a bounded fix round), and not the pilot.
