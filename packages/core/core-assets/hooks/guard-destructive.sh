...
# wrappers (`command`/`\git`/parens/quotes) ARE normalized before matching now.
# Still unhandled: `sh -c`, `eval`, base64/printf obfuscation, and escaped-quote
# edge cases inside quoted spans. This guard is a SEATBELT against accidental
...
# Normalized copy used ONLY by the hook-skip (1) and force-push (2) rules below.
# `$cmd` stays intact for the messages and the other rules. The transforms, in
# order, close verified bypasses without inventing matches (they only add
# boundaries / drop noise, so the change is always toward MORE blocking):
#   FIX A — strip single/double-quoted spans so a hyphen-word inside a commit
#           message (`git commit -m "add -notify option"`) can't trip the
#           combined-flag pattern. Escaped quotes inside quotes = accepted gap.
...
#    `-[a-zA-Z]*n[a-zA-Z]*` also catches `-n` folded into a combined short-flag
#    token (e.g. `git commit -qn -m x`), which a bare `-n` token missed. The
#    leading boundary accepts ; & | so `true;git commit --no-verify` (no space
#    after the separator) is caught too. The `git(… -opt …)*` middle allows git
#    global options between `git` and the subcommand (`git -c k=v commit …`).
