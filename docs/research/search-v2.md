...
| S22 | `docs/research/tgrep-como-funcionaba.md` §5.1, §6, §9; `scripts/py/mine-search-routing.py` | Evidencia local: doctrina sola 7.4%, guard 40.7%; el instrumento de campo con denominador corregido. Sustenta D19 y §9.5. |
...
- **Instrumento:** `scripts/py/mine-search-routing.py` con el denominador corregido (búsquedas reales; filtros y extracciones fuera del cociente) y `--desde/--hasta` fijados antes de mirar datos. Contar como vía de v2 las llamadas a `codegraph_explore` y a `tgrep search`; contar como escape `Grep` nativo, `rg`, `grep -r` y `git grep`.
