...
<!-- navori:managed id="leader-base" hash="f21a16d2" version="0.8.7" source="@navori/core" -->
...
pnpm format:check && pnpm check:links && pnpm check:render && pnpm check:assets && pnpm jscpd:check && pnpm semgrep:check && pnpm --filter @navori/website build && cd packages/cli && pnpm check:size && pnpm test:coverage && pnpm lint && pnpm typecheck    # full gate — before closing the session / creating the PR
```
