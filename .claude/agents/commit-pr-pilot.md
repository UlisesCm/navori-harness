...
<!-- navori:managed id="commit-pr-pilot-base" hash="543c34d9" version="0.6.1" source="@navori/core" -->
...
- You are on `main` or `main` or another protected branch → abort + ask for a branch.
...
> **Two branches, two roles:** `main` is the fork point (where you branched from); `main` is the PR's target branch (`gh pr create --base`). They're usually the same branch. Either way, the PR and its diff are always computed against `main` — the target, not the fork point.
...
git rev-parse --abbrev-ref HEAD                       # cannot be main or main
git fetch origin main --quiet
git log origin/main..HEAD --oneline           # must have ≥1 commit (or changes to commit)
git diff origin/main...HEAD --stat            # REAL scope of the PR (against the target)
...
1. **Gather context** (curated, don't dump the whole repo). The PR diff is against `main` (what GitHub will show):
   ...
   - **Commit drag** (only if `main` ≠ `main`): `git fetch origin main --quiet` and `git rev-list --count origin/main..origin/main`. If > 0, `main` is ahead of `main` and your PR drags those foreign commits: warn the user and suggest rebasing onto `main` before opening.
