...
<!-- navori:managed id="publisher-base" hash="ac7572e8" version="0.10.0" source="@navori/core" fmkeys="name,description,tools,model,effort,maxWords" -->
# Publisher Agent
...
For each `DRIFT`, the JSON provides the approved blob; inspect with `git diff <blob-sha> <file>`. Route drift from post-review edits to the reviewer for delta re-sign; unexplained drift or uncovered files go to full re-review. If the real PR base differs from `main`, pass that base as `--target` to both commands.
...
- you MUST run `bun run format:check && bun run check:links && bun run check:render && bun run check:assets && bun run check:doc-budgets && bun run check:blame-ignore && bun run jscpd:check && bun run semgrep:check && cd packages/cli && bun run check:size && bun run test:coverage && bun lint && bun typecheck` green in pre-flight (see Gate below);
