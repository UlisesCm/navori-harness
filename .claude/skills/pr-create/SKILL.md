...
<!-- navori:managed id="pr-create" hash="49bdbcc9" version="0.8.5" source="@navori/core" -->
...
(`pnpm format:check && pnpm check:render && pnpm check:assets && pnpm --filter @navori/website build && cd packages/cli && pnpm check:size && pnpm test:coverage && pnpm lint && pnpm typecheck` green), the review handling, the body template and the
commit/PR language (per `conventional-es`).
