...
| `pnpm format:check && cd packages/cli && pnpm test && pnpm lint && pnpm typecheck` green | Same — fresh exit 0 this turn | "the dev server runs", "build passed a while ago" |
...
- `CLAUDE.md` § Session closeout mentions `pnpm format:check && cd packages/cli && pnpm test && pnpm lint && pnpm typecheck` green. This skill adds "fresh evidence" rigor + covers UI / bug-fixed dimensions the quality gate doesn't touch.
