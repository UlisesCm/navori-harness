...
<!-- navori:managed id="verify-before-done-base" hash="a3108148" version="0.8.6" source="@navori/core" -->
# Verify Before Done
...
- `CLAUDE.md` § Session closeout mentions `pnpm format:check && pnpm check:render && pnpm check:assets && pnpm jscpd:check && pnpm semgrep:check && pnpm --filter @navori/website build && cd packages/cli && pnpm check:size && pnpm test:coverage && pnpm lint && pnpm typecheck` green. This skill adds "fresh evidence" rigor + covers UI / bug-fixed dimensions the quality gate doesn't touch.
