...
<!-- navori:managed id="verify-before-done-base" hash="3af462d0" version="0.6.0" source="@navori/core" -->
# Verify Before Done
...
| `pnpm format:check && cd packages/cli && pnpm test && pnpm lint` green | Same — fresh exit 0 this turn | "the dev server runs", "build passed a while ago" |
...
- `CLAUDE.md` § Session closeout mentions `pnpm format:check && cd packages/cli && pnpm test && pnpm lint` green. This skill adds "fresh evidence" rigor + covers UI / bug-fixed dimensions the quality gate doesn't touch.
