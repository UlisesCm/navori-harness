...
<!-- navori:managed id="ticket-intake" hash="c887d97e" version="0.5.1" source="@navori/core" -->
...
| 2 · AUDIT | `ticket-audit` agent | `audit_ticket_<ID>.md`: root cause/approach, files, alternatives, questions, tasks. **Gate: the user approves it.** |
...
| 5 · IMPLEMENT | ONE `implementer` agent | Reads CLAUDE.md → `audit_ticket_<ID>.md` → `explore_*.md` → applicable skill. Produces `impl_<feature>.md`. **Gate: `cd packages/cli && pnpm lint` green in the turn.** |
