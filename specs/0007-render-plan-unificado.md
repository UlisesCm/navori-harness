...
| **M6** | Arranque de engram en Codex | **Resuelto declarativamente (sin hook).** Spike (v0.145.0): los eventos `SessionStart`/`UserPromptSubmit`/`Stop` disparan (mismo patrón config.toml que PreToolUse) pero corren en **sandbox**; y el arranque real de engram (`session-start.sh` del plugin Claude Code) es un script complejo con server HTTP local + `jq`/`curl` que navori no debe duplicar (fuera de scope). Hallazgo clave: navori ya inyecta el **protocolo de engram + `mcp_server`** en ambos engines; lo único que faltaba vs el hook era la llamada automática a `mem_context` al inicio. Decisión (Ulises): reforzar la instrucción en lugar del hook — `engram-protocol.md` ahora hace `mem_context` el **primer paso obligatorio** de cada sesión, señalando que en hosts sin hook (Codex) esa llamada ES el arranque de la memoria. Aplica a Claude y Codex (prose compartido). | ✅ **2026-07-28** (declarativo) |
...
**Estado:** M3–M9 ejecutadas (2026-07-28). M6 resuelto declarativamente (refuerzo de prose, sin hook — el arranque de engram del plugin es un script con red/sandbox que navori no debe duplicar). Backlog §9 completo.
