...
- **R30** — Todo lo anterior salvo los comandos DEBERÁ quedar detrás de `harness.planTiers`
  (default `false`); en `false`, los contratos renderizados DEBERÁN quedar byte a byte como
  antes. Los comandos `plan classify|render|update|check` existen con cualquier valor.
