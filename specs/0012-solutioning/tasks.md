...
- [~] **T0.1** Baseline Escenario A — procesar BTBS-162 sin la capa, sobre
  bonum-webapp; registrar el plan que produce.
  → `.claude/progress/eval_baseline_A.md`
  **Cubre:** evidencia para R5, R6, R8.
- [ ] **T0.2** Extraer del baseline las racionalizaciones observadas (¿heredó la
...
- [ ] **T1.1** Crear `packages/core/core-assets/skills/solution-design.md` con el
...
- [ ] **T1.2** Registrar `"solution-design"` en `WORKFLOW_SKILLS`
...
- [ ] **T2.1** Insertar el sub-bloque R2-architectural en
...
- [ ] **T2.2** Reescribir la fase 4 de
  ...
  **Cubre:** R4, R12.
- [ ] **T2.3** Fortalecer `packages/core/core-assets/skills/spec-bootstrap.md`:
...
- [ ] **T3.1** Test de invariantes de contenido de la skill (tokens cortos, nunca
...
- [ ] **T3.2** Test de presupuesto: el delta del bloque `orquestacion` ≤120 palabras.
  **Cubre:** R21. **// Covers: R21**
- [ ] **T3.3** Gate completo: `pnpm format:check` (raíz) + `pnpm test` + `pnpm lint`.
  Actualizar los conteos que rompan (catalog; e2e si aplica).
- [ ] **T3.4** Render de prueba en repo temporal: la skill se materializa, el índice
  la lista con su trigger, el bloque trae las señales.
  **Cubre:** R1.
- [ ] **T3.5** Dogfood: `render --apply` sobre navori-harness, commit
  `chore(self-host)` separado.

## Batch 4 — Evals GREEN (con la capa)

- [ ] **T4.1** Escenario A (GREEN): re-correr BTBS-162 con la capa. PASS si el
  ...
  **Cubre:** R5, R6, R8.
- [ ] **T4.2** Escenario D: "cambia el texto del botón Save por Guardar" → cero
  artefacto de solutioning. **Cubre:** R3. **FAIL si genera artefacto.**
- [ ] **T4.3** Escenario E: diseño válido con un concern menor → veredicto
  `CONCERNS` y la implementación arranca. **Cubre:** R13, R15.
- [ ] **T4.4** Escenario F: dos interpretaciones con arquitecturas incompatibles →
  `BLOCKED` con los cuatro campos. **Cubre:** R14, R17.
- [ ] **T4.5** Registrar los cuatro resultados en `evals.md` (escenario · esperado ·
  observado · PASS/FAIL · ajuste derivado). **GATE H2:** Ulises revisa.

## Batch 5 — Cierre

- [ ] **T5.1** PR → `main` vía `commit-pr-pilot`.
- [ ] **T5.2** `mem_save` de las decisiones nuevas, entrada en `progress/history.md`,
