...
- [ ] **T2** (R9) — Escribir `scripts/mine-pr-routing.py` sobre el mismo molde que
  ...
  global los veredictos `block` / `allow` / `override` de `pr-publisher-confirm`, y cruza cada
  bloqueo contra el transcript de su sesión para ver si después hubo `Agent`/`Task` con
  `subagent_type: publisher` (numerador A) o un `override` (numerador B). **Normaliza
...
- [ ] **T4** (R1, R4) — En `core-assets/hooks/pr-publisher-confirm.sh`, sustituir la emisión de
  `permissionDecision: "ask"` por `exit 2` con la razón a stderr, en el idioma de `block()` de
  `guard-destructive`: primera línea `[navori] BLOCKED by pr-publisher-confirm: …`, y después las
  **dos** salidas, la delegación (`Agent` con `subagent_type: publisher`) y la ruta
  ...
  test: `pr-publisher-confirm.test.ts`::`bloquea el PR del hilo principal y nombra las dos salidas`
  —una aserción por salida: el `subagent_type: publisher` y la ruta literal del
...
- [ ] **T5** (R2, R3) — Los dos casos en los que el guard se hace a un lado. (a) El payload
  ...
  conservarlo al reescribir el final del script. (b) El `publisher` no es resoluble →
  ...
  **El resolvedor de (b) NO es `navori_worktree`.** Ese partial devuelve
  ...
  replicar lo que hace el host: **ascender por `.claude/agents/publisher.md`** desde el
  directorio sobre el que actúa el comando, nivel por nivel hasta la raíz del FS, y probar al
  final `~/.claude/agents/publisher.md`. `test -f` por nivel, sin forks, todo detrás del
  ...
  **El brazo que discrimina: `.claude/agents/` POR ENCIMA de la raíz de git.** Montaje completo,
  ...
  $T/.claude/agents/publisher.md && git init $T/repo && mkdir -p $T/repo/sub`, y el
  ...
  después. · test: `pr-publisher-confirm.test.ts`::`no toca el PR que viene de un subagente`,
...
- [ ] **T6** (R7, R8) — Fail-open y registro. Sin `jq`, con payload ilegible o con comando
  ...
  `pr-publisher-confirm.test.ts`::`deja pasar cuando el guard no puede decidir`,
...
- [ ] **T7** (R5) — Honrar `.claude/progress/pr-override` relativo a la raíz que resuelve
  ...
  `pr-publisher-confirm.test.ts`::`el override pasa una vez y se consume` con `// Covers: R5`
...
- [ ] **T8** (R6) — Centinela presente sin razón legible (vacío, solo espacios, ilegible):
  ...
  `pr-publisher-confirm.test.ts`::`un centinela sin razón no ampara nada` con `// Covers: R6`
...
- [ ] **T9** (R1) — Invertir el caso `NUNCA bloquea — el exit es 0 incluso cuando eleva` de la
  ...
  **Y los dos comentarios TS que quedarían mintiendo**, que ningún test cubre porque ese gate
  ...
  test: `pr-publisher-confirm.test.ts`::`el exit deja de ser 0 cuando hay ruta al publisher` con
  `// Covers: R1`
