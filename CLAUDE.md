...
- **Session start — go by what's in front of you, not by the host.** A startup hook covers only SOME openings: on Claude the engram plugin registers its `SessionStart` for `startup`, `clear` and `compact` — **not `resume`** — so a resumed session (`--resume`/`--continue`) opens with no memory at all while looking exactly like one that has it. So: where the memory context is already injected, work with what's injected — calling `mem_context` only re-fetches it. Where it is NOT — a resumed session, or a host with no startup hook (e.g. Codex) — that explicit call IS the memory startup and it's the mandatory first step; don't skip it.
