...
| 3 · Design | `solution-design` skill + ONE fresh-context `auditor` challenge | Only on an architectural signal (orchestration table), a `proceed-differently` verdict, or level 2 → skill `plan-advanced`. Produces `solution_<scope>.md` + `solution_review_<scope>.md`. **Gate: your verdict READY / CONCERNS / BLOCKED** — `CONCERNS` records the risk and moves on, only `BLOCKED` stops. No signal → straight to 4. |
| 4 · Implementation | ONE `implementer` agent, `verify-before-done` inside it, then `scribe` when `markdownRequests` is non-empty | Reads `audit_ticket_<ID>.md` (if 2 ran) → `solution_<scope>.md` (if 3 ran) → applicable skill; the encargo opens with `workplan: <feature>`. Produces `impl_<feature>.json` with fresh verification evidence at exit 0 (R2); `scribe` renders `impl_<feature>.md` from it and applies any `markdownRequests` in its own commit (R5, R7). **Gate: `cd packages/cli && bun lint` green in the turn.** No evidence → back here. |
| 5 · Review | `reviewer` agent + `review-diff` skill | `review_<feature>.md` — the complete diff, including the `scribe`'s commit when it ran. Two-pass; Pass 1 fails → `CHANGES_REQUESTED`, back to 4. `APPROVED` → continue. |
