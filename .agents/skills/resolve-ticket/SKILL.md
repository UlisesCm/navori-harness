...
<!-- navori:managed id="resolve-ticket" hash="a94c5607" version="0.9.0" source="@navori/core" -->
...
| 4 · Implementation | ONE `implementer` agent, `verify-before-done` inside it | Reads `audit_ticket_<ID>.md` (if 2 ran) → `solution_<scope>.md` (if 3 ran) → applicable skill. Produces `impl_<feature>.md` with fresh verification evidence at exit 0. **Gate: `cd packages/cli && bun lint` green in the turn.** No evidence → back here. |
