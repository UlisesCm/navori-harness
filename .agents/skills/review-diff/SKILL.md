...
<!-- navori:managed id="review-diff-base" hash="1e84dd5c" version="0.8.7" source="@navori/core" -->
...
- New or modified guard/policy (authorization, licence, rate limit) covering only some of the entry points that mutate the same resource → CRITICAL. Enumerate every one with evidence — via Code discovery routing's structural provider, or `locate-code` as fallback — and mark it covered, or justify each exclusion one by one. An occurrence count alone doesn't demonstrate the enumeration is complete.
...
<!-- navori:managed id="jscpd-review-extension" hash="f080c7a6" version="0.8.7" source="@navori/plugin-jscpd" -->
...
<!-- user: add here the bespoke rules of your stack/domain (the ones that are NOT generalizable). Suggestions:
     ...
     - Critical areas with their own rules: render/sync/backup writes and deletes in the user's repo, settings.json permissions, deny/ask rules and hooks, managed-block markers and the anti-rollback guard.
-->
