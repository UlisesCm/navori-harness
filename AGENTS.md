...
<!-- navori:managed id="navori-agents" hash="f9d60fe8" version="0.9.0" source="@navori/core" -->
...
- `architect` — Proposes what to build and why for a task with an architectural signal (shared abstraction, ownership change, contract, migration, hard-to-reverse decision). Applies `solution-design`, writes `solution_<scope>.md`. Never verdicts, decomposes or asks the user. Use when the orchestrator's architectural row fires.
